...
function parseArgs() {
  ...
  if (!parsed.input || !parsed.output) {
    console.error(
      "Usage: node scripts/import_sberquad.js --input <path> --output <path> [--limit 2000]"
    );
    process.exit(1);
  }
...
function loadSberQuAD(filePath) {
  ...
  // 1) Если корень — сразу массив Q&A (как в твоём примере)
  if (Array.isArray(parsed)) {
    if (!parsed.some(looksLikeQaItem)) {
      throw new Error(
        "Файл является массивом, но элементы не похожи на SberQuAD (нет context/question/qas)."
      );
    }
    return parsed;
  }

  // 2) Старый код для случаев, где корень — объект с полями rows/data/train/validation/...
  ...
  const dataArray = candidates.find(
    (arr) => Array.isArray(arr) && arr.some(looksLikeQaItem)
  );
...
function buildContent(context = "", answers) {
  const answerText =
    Array.isArray(answers?.text) && answers.text.length > 0
      ? answers.text[0]
      : "";
...
function normalizeEntry(topic, { topicIndex, paragraphIndex, qaIndex } = {}) {
  ...
  const question = topic.question ? topic.question : topic.q ? topic.q : "";
...
function convertToKnowledgeBase(data, limit) {
    ...
    if (Array.isArray(topic.paragraphs)) {
      topic.paragraphs.forEach((paragraph, paragraphIndex) => {
        paragraph.qas?.forEach((qa, qaIndex) => {
          entries.push(
            normalizeEntry(
              { ...qa, title: topic.title, context: paragraph.context },
              {
                topicIndex,
                paragraphIndex,
                qaIndex,
  ...
  if (finalEntries.length === 0) {
    throw new Error(
      "Не удалось сформировать ни одной записи: проверьте поля context/question/answers в исходных данных"
    );
  }
