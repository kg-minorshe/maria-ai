...
class SessionContext {
    ...
    addUserMessage(message, entities = [], emotionalState = 'neutral') {
        ...
        this.emotionalHistory.push({ state: emotionalState, timestamp: Date.now() });
    ...
    getRelevantContext(currentMessage) {
        ...
        return {
            ...
            emotionalContext: {
                currentState: recentEmotions[recentEmotions.length - 1]?.state || 'neutral',
    ...
    getRecentEntities(limit = 8) {
        ...
        return Array.from(entities.values())
            .sort((a, b) => {
                ...
                const scoreA = a.frequency * 0.7 + (Date.now() - a.lastMention) * -0.0001;
                const scoreB = b.frequency * 0.7 + (Date.now() - b.lastMention) * -0.0001;
...
class ReferenceResolver {
    ...
    resolve(message, recentEntities, currentTopic, conversationHistory = []) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            if (this.pronounPatterns[word]) {
                ...
                const candidate = this.findBestCandidate(pattern, recentEntities, currentTopic, i, words);
        ...
        for (const [reference, type] of Object.entries(this.contextualReferences)) {
            if (resolvedMessage.toLowerCase().includes(reference)) {
                const contextualInfo = this.resolveContextualReference(type, conversationHistory);
                if (contextualInfo) {
    ...
    findBestCandidate(pattern, recentEntities, currentTopic, wordIndex, allWords) {
        const scoredCandidates = recentEntities
            .filter(entity => this.matchesPattern(entity, pattern))
            .map(entity => ({
                ...entity,
                score: this.calculateCandidateScore(entity, currentTopic, wordIndex, allWords, pattern)
            }))
    ...
    calculateCandidateScore(entity, currentTopic, wordIndex, allWords, pattern) {
        ...
        const contextWords = allWords.slice(Math.max(0, wordIndex - 3), wordIndex + 3);
        if (contextWords.some(w => entity.name.toLowerCase().includes(w))) {
            score += 0.2;
        }

        // Бонус за соответствие грамматического рода
        if (pattern.type.includes('masculine') && this.isMasculine(entity.name)) score += 0.1;
        if (pattern.type.includes('feminine') && this.isFeminine(entity.name)) score += 0.1;
    ...
    matchesPattern(entity, pattern) {
        ...
        if (pattern.candidates.includes('person') && entity.type === 'person') return true;
        if (pattern.candidates.includes('object') && ['object', 'concept', 'topic'].includes(entity.type)) return true;
...
class TopicTracker {
    ...
    updateTopic(message, entities) {
        ...
        entities.forEach(entity => {
            ...
            this.topicWeights.set(entity.name, (this.topicWeights.get(entity.name) || 0) + weight);
...
class UserPreferences {
    ...
    updatePreferences(interaction) {
        ...
        } else if (interaction.message.includes('коротк') || interaction.message.includes('кратк')) {
...
module.exports = { DialogContextManager, SessionContext, EntityContext, ReferenceResolver };