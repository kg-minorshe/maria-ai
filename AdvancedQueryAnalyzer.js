...
class AdvancedQueryAnalyzer {
    constructor() {
        this.questionWords = [
            ...
            'сколько', 'кто', 'чей', 'чья', 'чьё', 'откуда', 'куда', 'отчего'
        ];
        
        this.conjunctions = [
            'и', 'а', 'но', 'или', 'либо', 'также', 'ещё', 'тоже', 'при этом', 
            'кроме того', 'помимо', 'вдобавок', 'дополнительно'
        ];
        
        this.relationWords = [
            'связан', 'связана', 'связано', 'относится', 'принадлежит', 'входит',
            'является', 'представляет', 'включает', 'содержит', 'состоит'
        ];
    ...
    analyzeComplexQuery(message, context = {}) {
        ...
        try {
            ...
            analysis.complexity = this.complexityClassifier.classify(analysis.normalizedMessage);
            analysis.isComplex = analysis.complexity !== 'simple';

            // 4. Разбиение на подзапросы
            analysis.subqueries = this.smartSplitIntoSubqueries(analysis.normalizedMessage, context);

            // 5. Извлечение сущностей с улучшенным алгоритмом
            analysis.entities = this.extractEntitiesAdvanced(analysis.normalizedMessage, context);

            // 6. Анализ отношений между сущностями
            analysis.relations = this.analyzeRelations(analysis.normalizedMessage, analysis.entities, context);

            // 7. Определение основного интента
            analysis.intent = this.identifyPrimaryIntent(analysis.normalizedMessage, analysis.subqueries);

            // 8. Классификация типов подзапросов
            analysis.queryTypes = analysis.subqueries.map(sq => this.classifyQueryTypeAdvanced(sq.query, sq.context));

            // 9. Проверка на неоднозначность
            analysis.ambiguity = this.ambiguityResolver.detectAmbiguity(analysis.normalizedMessage, analysis.entities);

            // 10. Извлечение временного и пространственного контекста
            analysis.temporalContext = this.extractTemporalContext(analysis.normalizedMessage);
            analysis.spatialContext = this.extractSpatialContext(analysis.normalizedMessage);
    ...
    smartSplitIntoSubqueries(message, context) {
        ...
        textParts.forEach((part, index) => {
            ...
            if (cleanPart.length > 8) { // минимальная длина подзапроса
                subqueries.push({
                    query: cleanPart,
                    index: index,
    ...
    extractEntitiesAdvanced(message, context) {
        ...
        if (global.knowledgeBase) {
            global.knowledgeBase.forEach(item => {
                ...
                if (messageLC.includes(title)) {
                    entities.push(this.createEntityInfo(item, title, messageLC, 'exact_match'));
                }
                ...
                if (titleWords.length > 1) {
                    const partialMatch = titleWords.some(word => 
                        word.length > 3 && messageLC.includes(word)
                    );
                    if (partialMatch && !entities.find(e => e.name === item.title)) {
                        entities.push(this.createEntityInfo(item, title, messageLC, 'partial_match'));
                    }
                }
                ...
                item.aliases.forEach(alias => {
                    ...
                    if (messageLC.includes(aliasLC) && !entities.find(e => e.name === item.title)) {
                        entities.push(this.createEntityInfo(item, aliasLC, messageLC, 'alias_match'));
                    }
                });
            });
        }
        ...
        if (context.recentEntities) {
            context.recentEntities.forEach(entity => {
                if (messageLC.includes(entity.name.toLowerCase()) && 
    ...
    createEntityInfo(item, matchedText, messageLC, matchType) {
        ...
        switch (matchType) {
            case 'exact_match': confidence = 0.95; break;
            case 'alias_match': confidence = 0.85; break;
            case 'partial_match': confidence = 0.65; break;
        }
    ...
    extractNamedEntities(message) {
        ...
        const patterns = {
            ...
            numbers: /\b\d+(?:[.,]\d+)?\s*(?:штук|процент|рубл|доллар|евро|метр|килограмм|литр)\b/g,
            emails: /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g,
            phones: /\b(?:\+7|8)[\s\-]?\(?\d{3}\)?[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}\b/g
        };
    ...
    analyzeRelations(message, entities, context) {
        ...
        this.relationWords.forEach(relationWord => {
            if (messageLC.includes(relationWord)) {
                const relatedEntities = this.findEntitiesNearRelation(messageLC, relationWord, entities);
        ...
        for (let i = 0; i < entities.length - 1; i++) {
            for (let j = i + 1; j < entities.length; j++) {
                ...
                if (distance < 100) { // символы близко друг к другу
            ...
            entities.forEach(entity => {
                if (entity.name !== context.currentTopic) {
                    relations.push({
                        type: 'contextual',
                        entities: [
                            { name: context.currentTopic, type: 'topic' },
                            entity
                        ],
                        confidence: 0.4,
    ...
    identifyPrimaryIntent(message, subqueries) {
        const intents = {
            'definition': {
                patterns: [/что такое/i, /что означает/i, /определение/i, /что это/i, /расскажи о/i],
                weight: 0.9
            },
            'comparison': {
                patterns: [/различие/i, /отличие/i, /сравни/i, /чем отличается/i, /разница/i, /vs/i, /против/i],
                weight: 0.85
            },
            'process': {
                patterns: [/как происходит/i, /как работает/i, /процесс/i, /алгоритм/i, /как сделать/i, /как получить/i],
                weight: 0.8
            },
            'location': {
                patterns: [/где находится/i, /где расположен/i, /местонахождение/i, /адрес/i],
                weight: 0.8
            },
            'time': {
            ...
            'list': {
                patterns: [/список/i, /перечисли/i, /какие/i, /виды/i, /типы/i, /варианты/i],
                weight: 0.7
            },
            'instruction': {
                patterns: [/как/i, /инструкция/i, /руководство/i, /пошагово/i, /этапы/i],
                weight: 0.7
            },
            'confirmation': {
    ...
    calculateAdvancedConfidence(analysis, context) {
        ...
        if (context.currentTopic && analysis.entities.some(e => e.name === context.currentTopic)) {
            confidence += 0.1;
        }
    ...
    isQuestion(text) {
        return this.questionWords.some(qw => text.toLowerCase().includes(qw)) || text.includes('?');
    }
    ...
    buildSubqueryContext(subquery, originalContext, index) {
        return {
            ...
            subqueryIndex: index,
            isSubquery: true,
            parentQuery: originalContext.originalMessage || originalContext.resolvedMessage
        };
    }
    ...
    classifyTemporalType(indicators) {
        ...
        const pastScore = indicators.filter(i => past.some(p => i.includes(p))).length;
        const presentScore = indicators.filter(i => present.some(p => i.includes(p))).length;
        const futureScore = indicators.filter(i => future.some(p => i.includes(p))).length;
    ...
    inferRelationType(entity1, entity2, messageLC) {
        ...
        if (entity1.type === 'knowledge_entity' && entity2.type === 'knowledge_entity') {
...
class SentimentAnalyzer {
    constructor() {
        this.positiveWords = [
            'хорош', 'отличн', 'класс', 'супер', 'здорово', 'круто', 'замечательн',
            'прекрасн', 'великолепн', 'спасибо', 'благодар'
        ];
        
        this.negativeWords = [
            'плох', 'ужас', 'отврат', 'глуп', 'тупо', 'дура', 'идиот',
            'не работа', 'не понима', 'бесполезн', 'провал'
        ];

        this.neutralWords = [
            'нормальн', 'обычн', 'средн', 'неплох', 'сойдет'
        ];
    }
...
class ComplexityClassifier {
    classify(message) {
        ...
        // Количество союзов
        const conjunctions = (message.match(/\b(?:и|а|но|или|также|кроме того)\b/gi) || []).length;
        ...
        const maxIndicator = Object.entries(indicators).reduce((max, [key, value]) => {
